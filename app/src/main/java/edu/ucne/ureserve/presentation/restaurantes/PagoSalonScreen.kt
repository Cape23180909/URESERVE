...
private const val PRECIO_SALON = "RD$ 15,000"
            ...
            Column(
                ...
                CardMetodosPago(
                    ...
                    onMetodoSeleccionado = { metodo ->
                        ...
                        DatosPersonalesSalonStore.metodoPagoSeleccionado = metodo
...
@Composable
private fun EncabezadoPagoSalon() {
    Row(
        modifier = Modifier.fillMaxWidth().padding(bottom = 16.dp),
...
private fun CardMetodosPago(
    ...
    Card(modifier = Modifier.fillMaxWidth(), colors = CardDefaults.cardColors(containerColor = Color.White)) {
        Column(Modifier.padding(16.dp)) {
            ...
            MetodoPagoSalonItem("Efectivo", R.drawable.dinero, metodoPagoSeleccionado == "Efectivo") {
                onMetodoSeleccionado("Efectivo")
            }
            MetodoPagoSalonItem(TARJETA_CREDITO, R.drawable.credito, metodoPagoSeleccionado == TARJETA_CREDITO) {
                onMetodoSeleccionado(TARJETA_CREDITO)
            }
            MetodoPagoSalonItem(TRANSFERENCIA_BANCARIA, R.drawable.trasnferencia, metodoPagoSeleccionado == TRANSFERENCIA_BANCARIA) {
    ...
    Card(modifier = Modifier.fillMaxWidth(), colors = CardDefaults.cardColors(containerColor = Color.White)) {
        Column(Modifier.padding(16.dp)) {
            ...
            Row(Modifier.fillMaxWidth(), Arrangement.SpaceBetween) {
                ...
                Text(PRECIO_SALON, color = Color.Black)
            }
            Row(Modifier.fillMaxWidth(), Arrangement.SpaceBetween) {
            ...
            Row(Modifier.fillMaxWidth(), Arrangement.SpaceBetween) {
                ...
                Text(PRECIO_SALON, fontWeight = FontWeight.Bold, color = Color.Black)
            }
        }
    }
}
...
@Composable
private fun CardDatosPersonales(persona: DatosPersonalesSalon) {
    Card(
...
private fun getRutaNavegacion(metodo: String, fecha: String): String {
    return when (metodo) {
        "Efectivo" -> "RegistroReservaSalon?fecha=$fecha"
        TARJETA_CREDITO -> "TarjetaCreditoSalon?fecha=$fecha"
        TRANSFERENCIA_BANCARIA -> "SalonTransferencia?fecha=$fecha"
        else -> "RegistroReservaSalon?fecha=$fecha"
    }
}
...
private fun procesarReserva(
    ...
    try {
        ...
        viewModel.confirmarReservacionSalonReuniones(params)
    } catch (e: Exception) {
        viewModel._uiState.update {
...
private fun obtenerHorasReserva(horaInicio: String, horaFin: String): Triple<String, String, Int> {
    return if (horaInicio.isBlank() || horaFin.isBlank()) {
        ...
        Triple(horaInicio, horaFin, calcularHoras(horaInicio, horaFin))
    }
}


@Composable
fun MetodoPagoSalonItem(titulo: String, iconRes: Int, seleccionado: Boolean, onClick: () -> Unit) {
    Row(
        Modifier
            .fillMaxWidth()
            .clickable { onClick() }
